function [lk der]=copu_reg_obj_Frank_simuGC(Y1,Y2,x,porder)


% copula GLM whole model
...
mu2=x(2*porder+2)+yhist*x(2*porder+3:v-1)';
...
indxtmp=find(indx00==1);
       ...
       fvec(indxtmp,2*porder+2)=-a4(indxtmp)./a5(indxtmp).*p2(indxtmp).*(1-p2(indxtmp)).*a7(indxtmp);
...
indxtmp=find(indx01==1);
       ...
       fvec(indxtmp,2*porder+2)=(a4(indxtmp).*p2(indxtmp).*(1-p2(indxtmp)).*a7(indxtmp))./(1-p1(indxtmp)-a5(indxtmp));
       for i=2*porder+3:v-1
           fvec(indxtmp,i)=fvec(indxtmp,2*porder+2).*yhist(indxtmp,i-2*porder-2);
       end       
...
indxtmp=find(indx10==1);
       ...
       fvec(indxtmp,2*porder+2)=(a4(indxtmp).*p2(indxtmp).*(1-p2(indxtmp)).*a7(indxtmp)-p2(indxtmp).*(1-p2(indxtmp)))./(1-p2(indxtmp)-a5(indxtmp));  
       for i=2*porder+3:v-1
           fvec(indxtmp,i)=fvec(indxtmp,2*porder+2).*yhist(indxtmp,i-2*porder-2);
       end      
...
indxtmp=find(indx11==1);      
       ...
       fvec(indxtmp,2*porder+2)=(p2(indxtmp).*(1-p2(indxtmp))-a4(indxtmp).*p2(indxtmp).*(1-p2(indxtmp)).*a7(indxtmp))./(p1(indxtmp)+p2(indxtmp)+a5(indxtmp)-1); 
       for i=2*porder+3:v-1
           fvec(indxtmp,i)=fvec(indxtmp,2*porder+2).*yhist(indxtmp,i-2*porder-2);
       end   
