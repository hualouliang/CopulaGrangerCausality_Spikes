function [lk der]=copu_reg_obj_Gauss_simuGC(Y1,Y2,x,porder)

% Gaussian copula GLM whole model
...
indxtmp=find(indx00==1);
       ...
       fvec(indxtmp,1)=-a6(indxtmp).*p1(indxtmp).*(1-p1(indxtmp))./a5(indxtmp);
       for i=2:2*porder+1
           fvec(indxtmp,i)=fvec(indxtmp,1).*yhist(indxtmp,i-1);
       end
       fvec(indxtmp,2*porder+2)=-a7(indxtmp).*p2(indxtmp).*(1-p2(indxtmp))./a5(indxtmp);
