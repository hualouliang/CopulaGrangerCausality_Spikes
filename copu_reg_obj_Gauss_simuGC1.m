function [lk der]=copu_reg_obj_Gauss_simuGC1(Y1,Y2,x,porder)

% Gaussian copula GLM reduced model 1
...
indxtmp=find(indx00==1);
       ...
       fvec(indxtmp,porder+2)=-a7(indxtmp)./a5(indxtmp).*p2(indxtmp).*(1-p2(indxtmp));
       for i=porder+3:v-1
...
indxtmp=find(indx01==1);
       ...
       fvec(indxtmp,porder+2)=(a7(indxtmp).*p2(indxtmp).*(1-p2(indxtmp)))./(1-p1(indxtmp)-a5(indxtmp));
       for i=porder+3:v-1
           fvec(indxtmp,i)=fvec(indxtmp,porder+2).*yhist(indxtmp,i-porder-2);
       end         
...
indxtmp=find(indx10==1);
       ...
       fvec(indxtmp,porder+2)=(a7(indxtmp).*p2(indxtmp).*(1-p2(indxtmp))-p2(indxtmp).*(1-p2(indxtmp)))./(1-p2(indxtmp)-a5(indxtmp)); 
       for i=porder+3:v-1
           fvec(indxtmp,i)=fvec(indxtmp,porder+2).*yhist(indxtmp,i-porder-2);
       end       
...
indxtmp=find(indx11==1);      
       ...
       fvec(indxtmp,porder+2)=(p2(indxtmp).*(1-p2(indxtmp))-a7(indxtmp).*p2(indxtmp).*(1-p2(indxtmp)))./(p1(indxtmp)+p2(indxtmp)+a5(indxtmp)-1); 
       for i=porder+3:v-1
           fvec(indxtmp,i)=fvec(indxtmp,porder+2).*yhist(indxtmp,i-porder-2);
       end   
