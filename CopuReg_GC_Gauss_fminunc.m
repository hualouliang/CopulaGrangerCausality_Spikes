function [gc12 gc21 para]=CopuReg_GC_Gauss_fminunc(Y1,Y2,porder,options)
...
gamma_init=corr(Y1',Y2');
% gamma_init=copulaparam('Gauss',abs(gamma_init));
gamma_init=log(log(2/(gamma_init+1)));
...
[x,fval]=fminunc(@(para) copu_reg_obj_Gauss_simuGC(Y1,Y2,para,porder),para_init,options);
toc        
...
tic
% options = optimset('GradObj','on','Display','iter','TolFun',1e-2,'TolX',1e-2,'LargeScale','off','MaxIter',200);
[x,fval]=fminunc(@(para) copu_reg_obj_Gauss_simuGC1(Y1,Y2,para,porder),para_init1,options);
toc        
...
tic
% options = optimset('GradObj','on','Display','iter','TolFun',1e-2,'TolX',1e-2,'LargeScale','off','MaxIter',200);
[x,fval]=fminunc(@(para) copu_reg_obj_Gauss_simuGC2(Y1,Y2,para,porder),para_init2,options);
toc        
